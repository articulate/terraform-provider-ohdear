...
build-macos:
	GOOS=darwin GOARCH=amd64 go build -o terraform-provider-ohdear_${VERSION}
	mkdir -p ~/.terraform.d/plugins/darwin_amd64/
	mv terraform-provider-ohdear_${VERSION} ~/.terraform.d/plugins/darwin_amd64/

build-linux:
	GOOS=linux GOARCH=amd64 go build -o terraform-provider-ohdear_${VERSION}
	mkdir -p ~/.terraform.d/plugins/linux_amd64/
	mv terraform-provider-ohdear_${VERSION} ~/.terraform.d/plugins/linux_amd64/
...
ship: build
	...
	&& if [ $$exists == "true" ]; then \
	  ...
	  aws s3 cp ~/.terraform.d/plugins/linux_amd64/terraform-provider-ohdear_${VERSION}  s3://${TERRAFORM_PLUGINS_BUCKET}/ --profile ${TERRAFORM_PLUGINS_PROFILE}; \
	fi
